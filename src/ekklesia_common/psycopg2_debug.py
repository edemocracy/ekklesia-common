...
def _make_statement_formatter(show_time, highlight, pygments_style, prefix=None, formatter_cls=None):
    def format_stmt(sql, timestamp=0, duration=0):
        if show_time:
            msg = "{:.2f} ({:.2f}ms): {}".format(timestamp, duration * 1000, sql.strip())
            if prefix:
                return prefix + msg
            else:
                return msg
    ...
    if highlight and pygments:
        ...
        def highlight_format_stmt(sql, timestamp=0, duration=0):
            return pygments.highlight(format_stmt(sql, timestamp, duration), lexer, formatter)
...
class StatementHistory(object):
    ...
    def format_statement(
        ...
        highlight_format_stmt = _make_statement_formatter(show_time, highlight, pygments_style, prefix, formatter_cls)
    ...
    def print_statements(self, show_time=True, highlight=True, pygments_style=DEFAULT_PYGMENTS_STYLE, prefix=None):
        if not self.entries:
            print("history is empty")
            return

        highlight_format_stmt = _make_statement_formatter(show_time, highlight, pygments_style, prefix)
