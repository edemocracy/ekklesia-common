...
let
  ekklesia-common = import ../. { inherit sources; };
  deps = ekklesia-common.deps;
  inherit (deps) pkgs gunicorn python lib;
  pythonpath = "${python}/lib/python3.7/site-packages,${ekklesia-common}/lib/python3.7/site-packages";
  ...
  runGunicorn = pkgs.writeShellScriptBin "run" ''
    ...
    ${gunicorn}/bin/gunicorn -c ${gunicornConf} \
      "ekklesia_common.app:make_wsgi_app(settings_filepath='$app_config')"
  '';

in pkgs.buildEnv {
  name = "ekklesia-common-serve-app";
  paths = [ runGunicorn ];
}
